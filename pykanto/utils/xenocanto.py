"""
Download recordings and metadata from xeno-canto programmatically.
"""
# Code modified from https://github.com/ntivirikin/xeno-canto-py
...
def metadata(filt: List[str], out_directory: str | os.PathLike) -> Path:
    """
    Download recording metadata for a given query.
    ...
    Args:
        ...
        out_directory (str | os.PathLike): Where to save the metadata.
            ...
            If os.PathLike: a metadata folder will be created in the provided
            directory.
...
def download(
    filt: List[str], out_directory: str | os.PathLike = "dataset"
) -> None:
    """
    ...
    print(
        f"Found {len(recordings)} files matching your query, "
        "download will start:"
    )
    ...
    if len(existing_files):
        num_form = (
            ["is", "was"] if len(existing_files) == 1 else ["are", "were"]
        )
        print(
...
def gen_meta(out_directory: str | os.PathLike = "dataset"):
    ...
    print(
        "You can find the library metadata file at "
