...
from bokeh.models import (BoxSelectTool, CustomJSHover, Div, HoverTool,
                          LassoSelectTool, Legend)
...
if 'sphinx' in sys.modules:
    ...
    if debug:
        warnings.warn('Debug is set to true. The app will not work '
    ...
    if len(remaining_indvs) == 0:
        raise KeyError(f"All IDs in {indv_list} have already been checked. "
                       "Close the app.")
    ...
    def get_legend_items():
        ...
        legend_items = [
            LegendItem(
                ...
                index=i) for i, label in enumerate(labs)
    ...
    # Add label button
    label_button = Dropdown(label='Label', menu=labs,
                            css_classes=['label_button'])
    ...
    def next_plot(event):
        ...
        remaining_indvs = [indv for indv in indv_list if indv not in getattr(
            dataset.DIRS, dataset_labels)['already_checked']]
