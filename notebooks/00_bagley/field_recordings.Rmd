...
```{r pkg, message=FALSE}
setwd("/media/nilo/Data/dphil/field_recordings/")
library(tidyverse)
library(lubridate)
library(ggmap)
```
...
file_gps <- list.files(path = "./data/", full.names = TRUE, pattern = ".gpx$") # make sure that there is only one .gpx file in this directory

file_wav_new <- list.files(path = "./data/", full.names = FALSE, pattern = ".wav$") %>%
  grep(., pattern = "_", inv = T, value = T) %>%
  sort(., decreasing = FALSE)

file_wav_existing <- list.files(path = "./data/", full.names = FALSE, pattern = ".wav$") %>%
...
## Dis my key, remove before sharing
