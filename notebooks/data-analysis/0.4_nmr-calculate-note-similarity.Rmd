...
#DTW
res$sound.files <-
  lapply(res$sound.files, function(x)
    gsub("\\..*", "", x))
...
res %>% slice(36) %>% gather() %>% slice(3:nrow(.)) %>% select(value) %>%  plot()

# Calculate DTW distances
system.time(dtw_dist <- proxy::dist(res[c(-1,-2)], method = "DTW_BASIC", norm = "L2", window.size = 10)) # series are of same length so distances are symmetrix even with a window applied
dtw_dist_matrix =as.matrix(dtw_dist)
rownames(dtw_dist_matrix) = res$sound.files
colnames(dtw_dist_matrix) = res$sound.files
heatmap(dtw_dist_matrix)

distst = as.dist(dtw_dist)

hclust(distst, method="centroid")


data("uciCT")

pc <- tsclust(CharTraj, type = "partitional", k = 10L,
