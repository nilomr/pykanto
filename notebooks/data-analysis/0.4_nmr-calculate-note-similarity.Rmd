...
# Integrate with above at some point
...
dendrogram = hclust(DTWdist, method = "average", xdist = TRUE)
clusters <-
  cutreeDynamic(
    dendrogram,
...
shared_matrix = sapply(unique(colnames(shared_matrix)), function(x) rowSums(shared_matrix[,grepl(x, colnames(shared_matrix))]))
...
nestbox_coords = read_csv(coords_dir) %>% filter(nestbox %in% summary_songs$bird) # only keep relevant birds
new_order <-
  sapply(summary_songs$bird, function(x, nestbox_coords) {
...
```{r}
grph = melt(shared_matrix, varnames = c("from", "to")) %>% rename(weight = value) %>% filter(weight != 0) 
graph = as_tbl_graph(grph) %>% mutate(x = coords$x, y =  coords$y)
