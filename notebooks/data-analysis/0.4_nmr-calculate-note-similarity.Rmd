...
a_vs_s_distance = distances_by_bird %>%
  ...
  theme(
    ...
    aspect.ratio = 1,
    plot.title=element_text(hjust=0, vjust=2.4, face='bold', size = 18, lineheight = .8),
...
grph = n_shared_songs %>% 
  ...
  filter(weight != 0) # %>% filter(s_dist < 2000) # %>% mutate(weight = 1) # 'remove' weigths
...
network_plot = ggraph(graph,  x = x, y = y) + 
  geom_edge_link(alpha = .9, colour='black', edge_width=.02) +
  geom_node_point(colour='black', alpha = 0.6) +
  theme(
    panel.background = element_rect(fill = '#f2f1f0', colour = NA),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    ...
    plot.subtitle=element_text(hjust=0, vjust=3, size = 15),
    text = element_text(size = 15)) +
  labs(title = "Song-sharing network")
...
network_map = wytham_map %>%
  ggmap(base_layer = ggraph(graph,  x = x, y = y), darken = c(0.4, "white")) %>% +
  coord_cartesian(xlim=c(-1.350288,-1.294670), ylim=c(51.752327,51.785683)) +
  geom_edge_link(alpha = .4, colour='black', edge_width=.03) +
  geom_node_point() +
...
ggraph(graph,  x = x, y = y) +
  geom_edge_link(alpha = .01, colour='white', aes(width = weight)) +
  geom_node_point() +
  theme_graph(background = 'grey20') +
  coord_fixed()
...
meanfreq_plot = freqs_by_bird %>%
  ggplot(aes(x = x, y = y, colour = mean_freq)) + 
  geom_point(size = 5) + 
  scale_colour_continuous_sequential(palette = 'Inferno', rev=F) +
  guides(colour = guide_colourbar(barwidth = 8, barheight = 1, ticks = FALSE, title.vjust = 0.858)) +
  labs(x = NULL, y = NULL) + 
  guides(x = "none", y = "none") +
theme(
    panel.background = element_rect(fill = '#e3ddd5', colour = NA),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    axis.ticks.y = element_blank(),
    axis.ticks.x = element_blank(),
...
theme(
    ...
    plot.margin = unit(c(1,1,1,1), "cm"),
    text = element_text(size = 14)) +
  labs(title = "Average minimum frequency",
       color = "kHz")

maxfreq_plot = freqs_by_bird %>%
  ggplot(aes(x = x, y = y, colour = max_freq)) + 
  geom_point(size = 5) + 
  scale_colour_continuous_sequential(palette = 'Inferno', rev=F) +
  guides(colour = guide_colourbar(barwidth = 8, barheight = 1, ticks = FALSE, title.vjust = 0.858)) +
  labs(x = NULL, y = NULL) + 
...
theme(
    ...
    plot.margin = unit(c(1,1,1,1), "cm"),
    text = element_text(size = 14)) +
  labs(title = "Average maximum frequency",
       color = "kHz")

freqs_map = maxfreq_plot +  meanfreq_plot + minfrequ_plot

ggsave(
  freqs_map,
  path = figures_dir,
  filename = "freqs_map.png",
  width = 40,
  height = 15,
  units = "cm",
  dpi = 350
...
freqs_by_bird %>% ggplot(aes(x= mean_freq, fill = as.factor(section))) + geom_density(alpha=0.3)
