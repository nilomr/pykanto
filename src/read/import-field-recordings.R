...
#  The code below is by Stuart K. Grange and uses Phil Harvey's Perl -exiftool- to extract a 
...
detect_exiftool <- function() {
  ...
  if (length(text) == 0 || !grepl("exiftool", text)) {
    stop("'exiftool' system program not detected. Please install 'exiftool'.", call. = FALSE)
  }
