...
from pykanto.utils.custom import parse_sonic_visualiser_xml, chipper_units_to_json
...
def new_dataset(DIRS):
    params = Parameters(
        window_length=512, hop_length=32, n_fft=2048, num_mel_bins=240,
        sr=22050, top_dB=65, highcut=10000, dereverb=True, song_level=True)
...
def test_chipper_units_to_json(DIRS):
    ...
    assert all([True for file in testfiles
                if 'onsets' in read_json(file)]) == True
