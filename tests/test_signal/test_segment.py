...
def test_SegmentMetadata(metadata, audio_section):
    segment = SegmentMetadata(metadata, audio_section,
                              2, 48000, Path('out')/"test.wav")
...
def test_segment_is_valid(
    ...
    assert segment_is_valid(metadata, i, min_duration,
                            min_freqrange, labels_to_ignore) == expected
